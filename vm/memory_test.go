...
func TestMemory_Set4(t *testing.T) {
	memory := NewMemory()
	...
	tests := []struct {
		...
		{0x04, []byte{0x00, 0x01, 0x02, 0x03}},
		{0x08, []byte{0x00, 0x01, 0x02, 0x03}},
		{0x0C, []byte{0x00, 0x01, 0x02, 0x03}},
	}
	...
	testsExpected := []struct {
		...
		{0x00, []byte{0x00, 0x01, 0x02, 0x03}, 4},
		{0x04, []byte{0x00, 0x01, 0x02, 0x03}, 4},
		{0x08, []byte{0x00, 0x01, 0x02, 0x03}, 4},
		{0x0C, []byte{0x00, 0x01, 0x02, 0x03}, 4},
	}

	memory.Resize(16)

	for _, test := range tests {
		memory.Set4(test.offset, test.data)
	}
...
func TestMemory_Set4_dynamic(t *testing.T) {
	memory := NewMemory()
	...
	testsExpected := []struct {
		...
		{0x00, []byte{0x00, 0x01, 0x02, 0x03}, 4},
		{0x04, []byte{0x00, 0x00, 0x01, 0x02}, 4},
		{0x08, []byte{0x00, 0x00, 0x00, 0x01}, 4},
		{0x0C, []byte{0x00, 0x00, 0x00, 0x00}, 4},
	}
...
func TestMemory_Data(t *testing.T) {
	...
	testBytes := []struct {
		...
		{0x00, []byte{0x00, 0x01, 0x02, 0x03}, 4},
		{0x04, []byte{0x04, 0x05, 0x06, 0x07}, 4},
		{0x08, []byte{0x08, 0x09, 0x0A, 0x0B}, 4},
		{0x0C, []byte{0x0C, 0x0D, 0x0E, 0x0F}, 4},
	}
